function script(){
    ...
    const resizeP = e => ourP.style.width = (([...ourContainer.children].reduce((accum, child)=>accum-child.getBoundingClientRect().width, ourContainer.getBoundingClientRect().width)/ourContainer.children.length) + ourContainer.firstElementChild.getBoundingClientRect().width+ourContainer.children[1].getBoundingClientRect().width) + 'px';
    ...
    const alignText = e => text.setAttribute('x', `${50 - text.getBoundingClientRect().width / 2}px`)
    ...
    const circInterval = setInterval(e => {
        ...
        polyline.setAttribute('points', `${polyline.getAttribute('points')} ${coordinate}`)
        text.innerHTML = `${Math.floor((count / coordinates.length) * 100)}%`;
        alignText()
        count+=6
        if(Math.floor((count/coordinates.length))*100>=(progressNum || 100)){ clearInterval(circInterval);text.innerHTMl = '100%'}
    },25)
}
